...
class MixedOp(nn.Module):
  ...
  def __init__(self, C, stride):
    ...
    for primitive in PRIMITIVES:
      ...
      self._ops.append(op)
      self._bn = nn.BatchNorm2d(len(PRIMITIVES) * C//4, affine=True)
  ...
  def forward(self, x):
    ...
    tmp = [op(xtemp) for op in self._ops]
    bn = self._bn(torch.cat(tmp, 1))
    temp1 = sum(bn.chunk(len(PRIMITIVES), dim=1))
...
class Cell(nn.Module):
  ...
  def __init__(self, steps, multiplier, C_prev_prev, C_prev, C, reduction, reduction_prev):
    ...
    for i in range(self._steps):
      for j in range(2+i):
        ...
        op = MixedOp(C, stride)
        self._ops.append(op)
      bn = nn.BatchNorm2d((2+i)* C, affine=True)
      self._bns.append(bn)
  ...
  def forward(self, s0, s1):
    ...
    for i in range(self._steps):
      ...
      # s = sum(self._bns[i].weight.data.chunk(len(states),dim=0)[j].unsqueeze(1).unsqueeze(2).unsqueeze(0)*self._ops[offset + j](h) for j, h in enumerate(states))##
      tmp =[self._ops[offset + j](h) for j, h in enumerate(states)]
      tmp1 = [torch.mean(self._bns[i].weight.data.abs().chunk(len(states), dim=0)[j]) for j, h in enumerate(states)]
      bn = self._bns[i](torch.cat(tmp, 1))
      tmp3 = bn.chunk(i+2, dim=1)
      s = sum( tmp1[k]*tmp3[k] for k in range(i+2) )
      offset += len(states)
      states.append(s)
